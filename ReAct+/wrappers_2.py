...
HOTPOTQA_SPLIT_FILE = {
    ...
    "dev": "2wiki_types/simplified.json",
...
class HotPotQAWrapper(gym.Wrapper):
    def __init__(self, env, split, local_rank=-1, total_rank=-1):
        super().__init__(env)
        ...
        if local_rank < 0 and total_rank < 0:
            ...
            full_data = json.load(open(data_file))
            each_len = math.ceil(len(full_data) / total_rank)
            if local_rank == total_rank - 1:
                self.data = full_data[local_rank * each_len:]
            else:
                self.data = full_data[local_rank * each_len:local_rank * each_len + each_len]
    ...
    def get_metrics(self, info):
        if info['answer'] is not None:
            ...
            return {'reward': em, 'em': em, 'f1': f1, 'cover_em': cover_em, }
        return {'reward': 0, 'em': 0, 'f1': 0, 'cover_em': 0}

    def step(self, **inputs):
        # TODO: first step obs does not have question.
        obs, _, done, info = self.env.step(**inputs)
...
class LoggingWrapper(gym.Wrapper):
    ...
    # <LoggingWrapper<HotPotQAWrapper<WikiEnv instance>>>
    def reset(self, seed=None, return_info=False, options=None, idx=None):
        ...
        self.traj = {"observations": [observation], "actions": [], "thoughts": []}
        return output

    def step(self, **inputs):
        obs, reward, done, info = self.env.step(**inputs)
        self.traj["observations"].append(obs)
        self.traj["actions"].append(inputs["action"])
        if done:
    ...
    def update_record(self):
        if len(self.traj) > 0:
            ...
            self.traj = {"observations": [], "actions": [], "thoughts": []}
